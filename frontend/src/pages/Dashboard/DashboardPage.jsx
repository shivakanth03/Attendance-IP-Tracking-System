import api from '../../api/axios';
import { useAuth } from '../../context/AuthContext';
import { useEffect, useState } from 'react';
import { useNavigate, useLocation } from 'react-router-dom';

const navItems = [
  { label: 'Dashboard',  path: '/dashboard',  icon: 'M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z' },
  { label: 'Attendance', path: '/attendance', icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4' },
  { label: 'Students',   path: '/students',   icon: 'M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75' },
  { label: 'QR Scanner', path: '/qr',         icon: 'M3 3h7v7H3zM14 3h7v7h-7zM3 14h7v7H3zM17 14h.01M14 14h.01M17 17h.01M14 17h.01M17 20h.01M20 14h.01M20 17h.01M20 20h.01' },
  { label: 'Network',    path: '/network',    icon: 'M5 12.55a11 11 0 0114.08 0M1.42 9a16 16 0 0121.16 0M8.53 16.11a6 6 0 016.95 0M12 20h.01' },
  { label: 'Reports',    path: '/reports',    icon: 'M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z' },
];

export default function DashboardPage() {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const location = useLocation();
  const [stats, setStats] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    api.get('/api/dashboard/stats')
      .then(res => {
        // ApiResponse wrapper: { success, message, data: {...} }
        const data = res.data?.data ?? res.data;
        setStats(data);
      })
      .catch(err => {
        console.error('Dashboard stats error:', err);
        setError('Could not load stats');
      })
      .finally(() => setLoading(false));
  }, []);

  const userName = user?.fullName || user?.email || 'User';
  const userInitial = userName[0].toUpperCase();

  return (
    <div className="dash-root">
      {/* Sidebar */}
      <aside className="sidebar">
        <div className="sidebar-brand">
          <div className="sidebar-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2" strokeLinecap="round" />
              <rect x="9" y="3" width="6" height="4" rx="1" />
              <path d="M9 12l2 2 4-4" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </div>
          <span>SmartAttend</span>
        </div>

        <nav className="sidebar-nav">
          {navItems.map(item => (
            <button
              key={item.label}
              className={`nav-item${location.pathname === item.path ? ' active' : ''}`}
              onClick={() => navigate(item.path)}
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d={item.icon} strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span>{item.label}</span>
            </button>
          ))}
        </nav>

        <div className="sidebar-user">
          <div className="user-avatar-sm">{userInitial}</div>
          <div className="user-info-sm">
            <p className="user-name-sm">{userName}</p>
            <p className="user-role-sm">{user?.role}</p>
          </div>
        </div>

        <button className="sidebar-logout" onClick={logout}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" strokeLinecap="round" strokeLinejoin="round" />
            <polyline points="16 17 21 12 16 7" />
            <line x1="21" y1="12" x2="9" y2="12" />
          </svg>
          <span>Logout</span>
        </button>
      </aside>

      {/* Main */}
      <main className="dash-main">
        {/* Header */}
        <header className="dash-header">
          <div>
            <h1>Dashboard</h1>
            <p>Welcome back, <strong>{userName}</strong> &bull; <span className="role-badge">{user?.role}</span></p>
          </div>
          <div className="header-avatar">{userInitial}</div>
        </header>

        {/* Stats Cards */}
        {loading ? (
          <div className="stats-grid">
            {[1,2,3,4].map(i => <div key={i} className="stat-skeleton"></div>)}
          </div>
        ) : error ? (
          <div className="error-box">{error}</div>
        ) : (
          <div className="stats-grid">
            <StatCard
              label="Total Students"
              value={stats?.totalStudents ?? 0}
              color="blue"
              icon="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2M9 11a4 4 0 100-8 4 4 0 000 8zM23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"
            />
            <StatCard
              label="Active Sessions"
              value={stats?.activeSessions ?? 0}
              color="green"
              icon="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"
            />
            <StatCard
              label="Today's Attendance"
              value={`${stats?.overallAttendancePercentage?.toFixed(1) ?? 0}%`}
              color="purple"
              icon="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414"
            />
            <StatCard
              label="Faculty Count"
              value={stats?.totalFaculty ?? 0}
              color="orange"
              icon="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"
            />
            <StatCard
              label="Departments"
              value={stats?.totalDepartments ?? 0}
              color="teal"
              icon="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"
            />
            <StatCard
              label="Online Users"
              value={stats?.onlineUsers ?? 0}
              color="rose"
              icon="M5 12.55a11 11 0 0114.08 0M1.42 9a16 16 0 0121.16 0M8.53 16.11a6 6 0 016.95 0M12 20h.01"
            />
          </div>
        )}

        {/* Quick Actions */}
        <div className="quick-actions">
          <h2>Quick Actions</h2>
          <div className="actions-grid">
            {[
              { label: 'Mark Attendance', desc: 'Start QR-based attendance session', icon: '✓', path: '/attendance', color: 'green' },
              { label: 'Scan QR Code', desc: 'Scan student QR for attendance', icon: '⊞', path: '/qr', color: 'blue' },
              { label: 'View Reports', desc: 'Download attendance reports', icon: '📊', path: '/reports', color: 'purple' },
              { label: 'Network Status', desc: 'View connected devices & IP logs', icon: '📡', path: '/network', color: 'orange' },
            ].map(action => (
              <button key={action.label} className={`action-card action-${action.color}`} onClick={() => navigate(action.path)}>
                <span className="action-icon">{action.icon}</span>
                <div>
                  <p className="action-title">{action.label}</p>
                  <p className="action-desc">{action.desc}</p>
                </div>
              </button>
            ))}
          </div>
        </div>

        {/* System Status */}
        <div className="backend-status">
          <h2>System Status</h2>
          <div className="status-grid">
            {[
              { label: 'Spring Boot API', detail: 'localhost:8080', ok: true },
              { label: 'H2 In-Memory Database', detail: 'Connected', ok: true },
              { label: 'Socket Server', detail: 'ws://localhost:9090', ok: true },
              { label: 'WebSocket STOMP', detail: 'Active', ok: true },
            ].map(s => (
              <div key={s.label} className="status-item">
                <span className={`status-dot ${s.ok ? 'online' : 'offline'}`}></span>
                <span>{s.label} — <strong>{s.detail}</strong></span>
              </div>
            ))}
          </div>
          <div className="swagger-link">
            <a href="http://localhost:8080/swagger-ui/index.html" target="_blank" rel="noreferrer">
              Open Swagger UI (API Docs) →
            </a>
          </div>
        </div>
      </main>
    </div>
  );
}

function StatCard({ label, value, color, icon }) {
  return (
    <div className={`stat-card stat-${color}`}>
      <div className="stat-icon">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          <path d={icon} strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
      <div className="stat-info">
        <p className="stat-value">{value}</p>
        <p className="stat-label">{label}</p>
      </div>
    </div>
  );
}
